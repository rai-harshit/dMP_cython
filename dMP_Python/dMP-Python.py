...
def extract_frames(storage):
	video_file = "test.mp4"
	sensitivity = 0.05
	os.system('ffmpeg -i {} -vf  "select=gt(scene\\,{}), scale=224:224, showinfo" -vsync vfr {}%01d.jpg 2>&1 | findstr /r "pts_time:[0-9]*.[0-9]*" > timestamp.log '.format(video_file,sensitivity,storage))
...
def prepare_data(storage):
	...
	frameData = prepare_frameData(list1)
